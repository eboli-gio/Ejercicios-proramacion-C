	...
	P Resta_p(P t,P q)
    {
		...
		r.p=(H*)malloc((r.gr+1)*sizeof(H));
		if(r.p==NULL)return t;
		for(i=0;min>=i;i++)r.p[i]=DifH(t.p[i],q.p[i]);
		if(t.gr==max&&(min!=max))for(i=min;i<=max;i++)r.p[i]=t.p[i];	
		if(q.gr==max&&(min!=max))for(i=min;i<=max;i++)r.p[i]=Prod_escH(-1.0,q.p[i]);		
