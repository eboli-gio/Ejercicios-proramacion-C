...
#include "polizp.h"
	...
	int pot_k_zp(int a,int k,int w)
	  {
	  	int i,r=1;
	  	for(i=0;i<k;i++) r=multi_zp(r,a,w);
	  	return r;
	  }
	...
	int escribe_P_A(P q,FILE *arch)
	{
		...
		if(poli_cero(q))
		{
			if(q.p[0]==0)fprintf(arch,"%d",0);
			else fprintf(arch,"%d",q.p[0]);
			return 0;
		}
		for(i=q.gr;i>=0;i--)
		{
			if(q.p[i]!=0&&q.gr>=0)
			{
				if(i!=0)
				{
					if(q.p[i]!=1)fprintf(arch,"%+dX^%d ",q.p[i],i);
					else if(i==q.gr)fprintf(arch,"X^%d",i);
					else fprintf(arch,"+X^%d",i);
				}
				else fprintf(arch,"%+d",q.p[i]);
			}
		}
		fprintf(arch,"\n");        
        return 0;
	...
	int val_p(P t,int a,int w)
    {
    	...
    	{
    		aux=pot_k_zp(a,i,w);
    		r=suma_zp(r,multi_zp(t.p[i],aux,w),w);
		}
		return r;
	}
